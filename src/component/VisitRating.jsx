  ...
  return (
    <div className = "bg-white p-5 mb-4">
        ...
        <div>
          ...
          <div className = "flex mb-4">
            <span className = "flex items-center text-sm mr-2">
              맛<FaStar  className = "text-myblue"/>{data.avgTaste}
            </span>
            <span className = "flex items-center text-sm mr-2">
              가격<FaStar  className = "text-myblue"/>{data.avgPrice}
            </span>
            <span className = "flex items-center text-sm mr-2">
              서비스<FaStar  className = "text-myblue"/>{data.avgService}
            </span>
          </div>
      ...
      <div className = "flex justify-between mt-4 mb-4">
        ...
        <div className = "">
          <p className = "text-sm">이 음식점의 평가결과는 신뢰할 수 있을만큼 이루어졌습니다.
          </p>
          <p className = "text-xs text-mediumGray">홍보 및 비방 등 부적절한 평가는 평점산정에서 제외될수있습니다.</p>
        </div>
        <button className = "text-sm border border-black p-3">맛집 평가하고 300원까지 적립받기</button>
      </div>
      <Review
        setData = {setData}
      />
      <button
        className = "bg-lightGray h-16 w-full rounded-lg flex items-center justify-center mt-4"
